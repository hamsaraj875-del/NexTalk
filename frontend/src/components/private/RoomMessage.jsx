...
const RoomMessage = ({roomData}) => {
  ...
  useEffect(() => {
    ...
    const fetcher = async () => {
      try {
        const response = await fetch(`${import.meta.env.VITE_LINK}/chat/room/messages/?roomId=${encodeURIComponent(data._id)}`, {
          signal,
          method: "POST",
          credentials: "include",
        });
          ...
          {loader ? (
            ...
            <div className="w-full h-full overflow-y-scroll scrollbar-none flex flex-col gap-4 px-6 py-6">
              {data
                .filter(
                  (msg) =>
                    msg.receiverId === roomData.owner || msg.senderId === roomData.owner,
                )
                .map(({ senderId,  message, time }) => (
                  <div
