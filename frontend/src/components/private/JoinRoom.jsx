    ...
    const fetcher = async () => {
      try {
        const response = await fetch(
          `${import.meta.env.VITE_LINK}/chat/room/search?name=${encodeURIComponent(search.trim())}`,
          {
            signal,
            credentials: "include",
          },
      ...
      <div className="w-[380px] rounded-2xl border border-gray-700 bg-[#0d0b16] shadow-2xl">
        ...
        <div className="max-h-[350px] space-y-2 overflow-y-auto px-4 pb-4 scrollbar-none">
          {groupList?.map((group) => (
            <div
              key={group._id}
              className="group rounded-2xl border border-gray-800 bg-gradient-to-br from-white/[0.05] to-white/[0.02] p-4 transition-all duration-200 hover:border-purple-500/40 hover:shadow-lg hover:shadow-purple-900/10"
            >
