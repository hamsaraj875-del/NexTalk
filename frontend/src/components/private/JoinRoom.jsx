...
const JoinRoom = ({ setJoinRoom }) => {
  ...
  const find = async (searchValue) => {
    ...
    try {
      const response = await fetch(
        `${import.meta.env.VITE_LINK}/chat/room/search?name=${encodeURIComponent(searchValue.trim())}`,
        {
          credentials: "include",
        },
      );
  ...
  const requestGroup = async (group, password = "") => {
    ...
    try {
      const response = await fetch(
        `${import.meta.env.VITE_LINK}/chat/room/join`,
        {
              ...
              >
                <div className="flex items-center justify-between gap-3">
                  ...
                  {group.type == "public" && (
                    <button
                      onClick={() => {
                        requestGroup(group, passwords[group._id] || "");
                      }}
